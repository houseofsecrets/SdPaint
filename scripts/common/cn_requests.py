...
def fetch_detect_image(state, image, width, height):
    ...
    if response.status_code == 200:
        r = response.json()
        return {"status_code": 200, "image": r['images'][0], "info": r["info"]}
    else:
        return {"status_code": response.status_code}
...
def fetch_img2img(state):
    ...
    if response.status_code == 200:
        r = response.json()
        return {"image": r['images'][0], "info": r["info"]}
    else:
        return {"status_code": response.status_code}
...
def post_request(state):
    ...
    if response.status_code == 200:
        ...
        if len(r['images']) == 1 + ignore_images:
            ...
            return {"status_code": 200, "batch_images": r['images'][:-ignore_images], "info": r["info"]}
    else: return {"status_code": response.status_code}